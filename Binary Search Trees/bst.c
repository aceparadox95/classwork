...
struct Node *_addNode(struct Node *cur, TYPE val)
{
		...
		if (compare(val , cur->val) == -1)  // if newValue is LESS, go left
		{
			//recall function on left node
			cur->left = _addNode(cur->left, val);
		}
		else // if newValue is GREATER, go right
		{
			//recall function on right node
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	assert(tree != 0);
	assert(val != NULL);
	struct Node *temp = tree->root; // creates ‘iterator’ and initializes it
	while (temp != 0) // checks input value against roots value
	{
		if (compare(val, temp->val) == 0)
			return 1; // Value was found, exited while loop
		else if (compare(val, temp->val) == 1)
			temp = temp->right; //go down right side if greater
		else // (d < temp)
			temp = temp->left;//go down left side if less than
	}
	return 0; //value was never found 
		//return 0;
}
...
{
	while (cur->left != NULL) //Traverse through left child until stopping just before NULL left node
		cur = cur->left;
	return cur->val;
}
...
{
	if (cur->left == NULL) {			// Have reached the left-most node	
		struct Node *newNode = cur->right;//Save right child of this node
		free(cur); 				// Deallocate node
		return newNode; 				// Return right child
	}
	// Traverse through tree until reaching a leaf node on the left
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	{
		if (cur->right == 0)
		{		// No right child to traverse down
		...
		{
			// Replace value with left most value on right tree
			cur->val = _leftMost(cur->right);
			cur->right = _removeLeftMost(cur->right);
		}
	}
	else if (compare(val , cur->val) == -1) 		// Less than, go to the left
		cur->left = _removeNode(cur->left, val);
	else 	// Greater than, go to the right
