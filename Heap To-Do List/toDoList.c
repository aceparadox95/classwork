...
Description: This is a to-do list program that
...
void printList(DynArr *heap)
{
	...
	//sortHeap(newHeap);
	while (sizeDynArr(newHeap) > 0)
	{
		TYPE printTask = getMinHeap(newHeap);
		print_type(printTask);
		removeMinHeap(newHeap);
	}
	deleteDynArr(newHeap);
}
