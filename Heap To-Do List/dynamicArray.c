/*	Name: Ryan Cosby 
	...
	Description: This is the implementation file for the dynamic
	Array structure, including added functionality for bags, stacks,
	and heaps abstract data types             */
...
int _smallerIndexHeap(DynArr *heap, int i, int j)
{
	...
	if (compare(getDynArr(heap, i) , getDynArr(heap, j)) <= 0) //find lowest of the two children
		return i;
	return j;
}
...
void addHeap(DynArr *heap, TYPE val)
{
	...
	while (last != 0) // stops when the last value is index 0
	{
		lastParent = (last - 1) / 2;
		/*Compare value at parent to the current child, if its less then swap*/
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	{
		/* set smallChild to smaller of the two children (2 children)*/
		int smallChild = _smallerIndexHeap(heap, leftChild, rightChild);
		/* Check priority of smallChild against priority of parent child */
		if (compare(getDynArr(heap, smallChild), getDynArr(heap, pos)) == -1)
		{
			/* if SMALL CHILD LESS, then SWAP */
			swapDynArr(heap, pos, smallChild);
			_adjustHeap(heap, max, smallChild); //recursive call on smallChild
		}
	...
	else if (leftChild < max) /* we have one child */
	{
		...
		if (compare(getDynArr(heap, leftChild), getDynArr(heap, pos)) == -1)
		{
			...
			_adjustHeap(heap, max, leftChild); //recursive call
		}
...
void _buildHeap(DynArr *heap)
{
	...
	for (int i = (max / 2) - 1; i >= 0; i--)
		_adjustHeap(heap, max, i); //swaps at first non-leaf node each iteration as its built
}
/*
