...
{
	struct DynArr *r = createDynArr(g->numVertices);
	struct cirListDeque *c = malloc(g->numVertices * sizeof(Vertex*));
	initCirListDeque(c);
	addFrontCirListDeque(c,source);
	while (!isEmptyCirListDeque(c))
	{
		struct Vertex *tempVert = frontCirListDeque(c);
		removeFrontCirListDeque(c);
		if (!containsDynArr(r, tempVert))
		{
			addDynArr(r,tempVert);
			for (int i = 0; i < tempVert->numNeighbors; i++)
			{
				if (compare(tempVert->neighbors[i], destination) == 0)
					return 1;
				if (!containsDynArr(r, tempVert->neighbors[i]))
					addFrontCirListDeque(c, tempVert->neighbors[i]);
			}
		}
	}
	return 0;
		...
		if (!containsDynArr(r, tempVert))
		{
			...
			for (int i = 0; i < tempVert->numNeighbors; i++)
			{
				if (compare(tempVert->neighbors[i], destination) == 0)
					return 1;
				if (!containsDynArr(r, tempVert->neighbors[i]))
					addBackCirListDeque(c, tempVert->neighbors[i]);
			}
		}
	}
	return 0;
...
{
	struct Vertex *newLeft = (struct Vertex*) left;
	struct Vertex *newRight = (struct Vertex*) right;
	if (newLeft->label < newRight->label)
		return -1;
	else if (newLeft->label > newRight->label)
		return 1;
	return 0;
}