...
CONSTRAINT UNIQUE (name,subcategory)
)ENGINE=InnoDB;
...
INSERT INTO device (cid, name, received, isbroken) 
	VALUES ((SELECT id 
	FROM category_tbl 
	WHERE subcategory = 'maybe a tablet?'),'Samsung Atlas', '1970-01-02', true);
...
INSERT INTO os_support(did, osid, notes)
	VALUES ((SELECT id
	FROM device 
	WHERE name = 'Samsung Atlas'), (SELECT id
	FROM operating_system
	WHERE name = 'Android' AND version = '1.0'), 'Works poorly');
-- device: Samsung Atlas
-- os: Android 2.0
-- notes: NULL
INSERT INTO os_support(did, osid, notes)
	...
	WHERE name = 'Samsung Atlas'), (SELECT id
	FROM operating_system
	WHERE name = 'Android' AND version = '2.0'), NULL);
-- device: jPad
-- os: iOS 4.0
-- notes: NULL
INSERT INTO os_support(did, osid, notes)
	VALUES ((SELECT id
	FROM device 
	WHERE name = 'jPad'), (SELECT id
	FROM operating_system
	WHERE name = 'iOS' AND version = '4.0'), NULL);
